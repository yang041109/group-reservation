...
export default function MyReservationsPage() {
  ...
  useEffect(() => {
    ...
    // Auto-search if phone is saved
    if (savedPhone) {
      fetchReservations(savedPhone);
    }
  }, []);
  ...
  async function fetchReservations(searchPhone: string) {
    ...
    setSearched(true);
    try {
      const res = await fetch(`/api/reservations/check?userPhone=${encodeURIComponent(searchPhone)}`);
      if (res.ok) {
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-8">
      ...
      <div className="mt-6 space-y-3 rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        <p className="text-sm text-gray-500">전화번호로 예약을 조회합니다</p>
      ...
      {loading ? (
        ...
        <div className="mt-6 space-y-4">
          {reservations.map((r) => {
            const status = STATUS_LABEL[r.status] ?? { text: r.status, color: 'bg-gray-100 text-gray-700' };
              ...
              <div key={r.id} className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
                <div className="cursor-pointer" onClick={() => router.push(`/stores/${r.storeId}`)}>
                  <div className="flex items-center justify-between">
                    <h2 className="text-base font-semibold text-gray-900">가게 {r.storeId}</h2>
                    <span className={`rounded-full px-3 py-1 text-xs font-medium ${status.color}`}>
                      {status.text}
                    </span>
                  </div>
                  <div className="mt-2 space-y-1 text-sm text-gray-600">
                    <p>👥 {r.headcount}명 · 🕐 {r.timeBlock}</p>
                    <p>💰 {r.totalAmount.toLocaleString()}원</p>
                  </div>
                  {r.menus.length > 0 && (
                    <div className="mt-2 text-sm text-gray-500">
                      {r.menus.map((m, i) => (
                        <span key={i}>{m.name} ×{m.quantity}{i < r.menus.length - 1 ? ', ' : ''}</span>
                      ))}
                    </div>
                  )}
                </div>
                ...
                {r.status === 'CANCELED' && (
                  <div className="mt-3 text-xs text-gray-400">취소된 예약입니다</div>
                )}
              </div>
            );
          })}
